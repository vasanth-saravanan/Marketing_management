...
//services
import * as Action from '../../redux/Expected_Income/Action'

//root store
import { RootStore } from '../../redux/store'


export function ExpectedIncome({ navigation } : any) {
    ...
    const getNetInfo = (CompanyUrl : any, BusinessUnitId : any, pageno : number , SetState : any) => {
      ...
      NetInfo.fetch().then((status) => {
          if(status.isConnected == true){
            dispatch(Action.getExpIncome(CompanyUrl, BusinessUnitId, pageno, SetState))
          }else {
    ...
    useEffect(() => {
        dispatch(Action.getExpIncome(CompanyUrl, BusinessUnitId, 0, SetState))
    }, [])
        ...
        <View style={styles.backgroundContainer}>
            <View style={styles.header}>
                <View style={styles.headerRight}>
                    ...
                    <Text style={styles.salesDetails}>{constants.Labels.exp_income}</Text>
                </View>
            </View>
            <SegmentedControl
