...
//services
import * as Action from '../../redux/sales/Action'
...
export function Sales({ navigation } : any) {
    ...
    const getNetInfo = (CompanyUrl : any, BusinessUnitId : any, pageno : number , SetState : any) => {
      ...
      NetInfo.fetch().then((status) => {
          if(status.isConnected == true){
            dispatch(Action.getSales(CompanyUrl, BusinessUnitId, pageno, SetState))
          }else {
    ...
    useEffect(() => {
        dispatch(Action.getSales(CompanyUrl, BusinessUnitId, 0, SetState))
    }, [])
        ...
        <View style={styles.backgroundContainer}>
            <View style={styles.header}>
                <View style={styles.headerRight}>
                    ...
                    <Text style={styles.salesDetails}>{constants.Labels.sales_details}</Text>
                </View>
            </View>
            <SegmentedControl
