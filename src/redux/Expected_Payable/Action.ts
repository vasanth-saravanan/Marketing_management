...
var fromdate; var todate; var Payablelist; var Totalamount;

export const getExpPayable = (CompanyUrl: any, BusinessUnitId: any, time: any, SetState: any) => (dispatch: Dispatch<SalesDispatchType>) => {
    if (time == 0) {
        time = '1 D'
    } else if (time == 1) {
        time = '7 D'
    ...
    let params = 'FromDate=' + fromdate + '&ToDate=' + todate + '&BusinessUnitId=' + BusinessUnitId + '&Type=Payable' + '&Filter=' + time;


    axios.get(CompanyUrl + constants.URL.api_bank + params)
        .then(response => response.data)
        .then((responseJson) => {
            Payablelist = responseJson;
            ...
            var dateArr: any = [];
            var resultarray: any[] = [];
            Payablelist.forEach(function (item: any) {
            ...
            Totalamount = 0;
            for (var p = 0; p < Payablelist.length; p++) {
                Totalamount += Number(Payablelist[p].Amount);
            }
